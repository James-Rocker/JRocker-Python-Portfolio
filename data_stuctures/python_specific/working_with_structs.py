...
# then to unpack
print(struct.unpack(formatting, packed))  # python always returns the unpacked values as tuples

# let's get the size!
size = struct.calcsize(formatting)
print("Size in bytes: {}".format(size))
print("lets compare against an unstructured size", struct.unpack(formatting, packed).__sizeof__())
